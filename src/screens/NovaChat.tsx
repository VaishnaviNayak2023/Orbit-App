...
export default function NovaChat() {
  ...
  const speak = (text: string, langCode?: string) => {
    ...
    if ('speechSynthesis' in window) {
      ...
      utterance.pitch = 1.1; // Slightly more natural
      utterance.rate = 1.0;

      const targetLang = langCode ? (LANG_MAP[langCode] || langCode) : (LANG_MAP[detectedLang] || detectedLang);
  ...
  const initChat = useCallback(() => {
    ...
    try {
      ...
      chatRef.current = ai.chats.create({
        ...
        config: {
          systemInstruction: `You are Nova, an empathetic and organized virtual companion.
      ...
      chatRef.current._petName = pet.name;
    } catch (err) {
  ...
  useEffect(() => {
    if (!chatRef.current || chatRef.current._petName !== pet.name) {
      initChat();
    }
  }, [pet, initChat]);
  ...
  const handleSend = async (text: string) => {
    ...
    // Prime voice and clear existing speech
    primeVoice();
    if ('speechSynthesis' in window) {
      window.speechSynthesis.cancel();
    }
    ...
    try {
      if (!chatRef.current) {
        initChat();
        if (!chatRef.current) throw new Error("AI Assistant is initializing...");
      }
      ...
      let responseData;
      try {
        // Handle potential reasoning text if present (property .text should just be the result)
        const textContent = response.text || "";
        responseData = JSON.parse(textContent);
      } catch (e) {
        responseData = { text: response.text || "I'm having trouble phrasing that properly.", lang: 'en' };
      }
